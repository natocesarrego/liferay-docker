...
EOF
	...
	release.versions.test.other.dir=${HOME}/dev/projects/liferay-portal-7.2.x
...
function main {
	...
	sed \
		--expression "s/USER_EMAIL/${LIFERAY_NEXUS_PUBLISHER_GIT_USER_EMAIL}/" \
		--in-place \
		.gitconfig
	sed \
		--expression "s/USER_NAME/${LIFERAY_NEXUS_PUBLISHER_GIT_USER_NAME}/" \
		--in-place \
		.gitconfig
