...
function get_environment_descriptor {
	local descriptor="${SPINNER_LIFERAY_LXC_REPOSITORY_DIR}/automation/environment-descriptors/${LXC_ENVIRONMENT}.json"

	if [ ! -e "${descriptor}" ]
	then
		echo "The ${descriptor} environment-descriptor does not exist."

		exit "${LIFERAY_COMMON_EXIT_CODE_BAD}"
	fi

	grep -e "\"${1}\":" "${descriptor}" | cut -d '"' -f 4
}
