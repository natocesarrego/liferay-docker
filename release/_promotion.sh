...
function prepare_jars_for_promotion {
	...
	if [ -n "${nexus_repository_name}" ] && ([ -z "${RELEASE_NEXUS_REPOSITORY_USER}" || -z "${RELEASE_NEXUS_REPOSITORY_PASSWORD}" ])
	then
...
function prepare_poms_for_promotion {
	...
	if [ -n "${nexus_repository_name}" ] && ([ -z "${RELEASE_NEXUS_REPOSITORY_PASSWORD}" || -z "${RELEASE_NEXUS_REPOSITORY_USER}" ])
	then
