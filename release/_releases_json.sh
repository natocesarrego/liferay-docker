...
function generate_releases_json {
	...
	then
		...
		local status=$(_process_new_product)
	fi

	if [ "${status}" -eq "${LIFERAY_COMMON_EXIT_CODE_SKIPPED}" ]
	then
...
function _process_new_product {
	local releases_json="${_RELEASE_ROOT_DIR}/release-data/promotion/files/0000-00-00-releases.json"
