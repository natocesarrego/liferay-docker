...
function _process_new_product {
	...
	jq 'map(if .productGroupVersion == "'"${product_group_version}"'" and .product == "'"${LIFERAY_RELEASE_PRODUCT_NAME}"'" then .promoted = "false" else . end)' "${releases_json}" > tmp.json && mv tmp.json "${releases_json}"
