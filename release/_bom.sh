...
function generate_api_jars {
	...
	then
		TOMCAT_VERSION=$(grep 'catalina=' "${_PROJECTS_DIR}/liferay-portal-ee/lib/development/dependencies.properties" | cut -d ':' -f3)

		_manage_bom_jar "${_BUNDLES_DIR}/tomcat/lib/ext/portal-kernel.jar"

		_manage_bom_jar "${_BUNDLES_DIR}/tomcat/lib/ext/support-tomcat.jar"
...
function generate_pom_release_bom {
	...
	for artifact_file in $(
		find "${_BUNDLES_DIR}/osgi" "${_BUNDLES_DIR}/tomcat/webapps/ROOT/WEB-INF" "${_BUNDLES_DIR}/tomcat/lib/ext" -name '*.jar' | \
				sed \
					-e 's/\.jar$//' \
					-e "s@.*/@@" \
					-e "s@-@.@g" | \
