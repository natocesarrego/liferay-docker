...
function generate_pom_release_dxp_bom_third_party {
	...
	local compile_only_file_name="release.dxp.bom.compile.only-${_PRODUCT_VERSION}-${_BUILD_TIMESTAMP}.pom"

	local dependencies_included=()

	for dependency_property in "${dependencies_properties[@]}"
	do
		IFS=':' read -ra dependency_property_parts <<< "$(echo "${dependency_property}" | cut -d= -f2)"

		if [[ ${dependencies_included[@]} =~ "${dependency_property_parts[0]}${dependency_property_parts[1]}${dependency_property_parts[2]}" ]]
		then
			echo "${dependency_property_parts[0]}${dependency_property_parts[1]}"
			continue
		fi

		dependencies_included+=("${dependency_property_parts[0]}${dependency_property_parts[1]}${dependency_property_parts[2]}")

		if (grep -q "<artifactId>${dependency_property_parts[1]}</artifactId>" "${compile_only_file_name}" && grep -q "<groupId>${dependency_property_parts[0]}</groupId>" "${compile_only_file_name}")
		then
			continue
		fi
