...
function generate_javadocs {
	...
	git fetch upstream "refs/tags/${_PRODUCT_VERSION}:refs/tags/${_PRODUCT_VERSION}"
	...
	local release_info_version_file_suffix="-$(echo "${_PRODUCT_VERSION}" | cut -d '-' -f 2)"
	...
	ant \
		...
		-Dservice.pack.version.suffix="${release_info_version_file_suffix}" \
