...
function upload_to_docker_hub {
	...
	echo "_BASE_DIR = ${_BASE_DIR}"
	ls -la "${_BASE_DIR}"

	git status

	echo "_PROJECTS_DIR = ${_PROJECTS_DIR}"
	ls -la "${_PROJECTS_DIR}"
	...
	then
		...
		LIFERAY_DOCKER_IMAGE_FILTER="${_PRODUCT_VERSION}" LIFERAY_DOCKER_RELEASE_CANDIDATE="false" ./build_all_images.sh
	fi
