...
function add_fixed_issues_to_patcher_project_version {
	...
	do
		...
		then
			...
			lc_log ERROR "Unable to add fixed issues to Liferay Patcher project ${2}. ${update_fixed_issues_response}"
...
function add_patcher_project_version {
	...
	then
		...
		lc_log ERROR "Unable to add Liferay Patcher project ${patcher_project_version}. ${add_by_name_response}"
...
function _update_bundles_yml {
	...
	if (yq eval ".\"${product_version_key}\" | has(\"${_PRODUCT_VERSION}\")" "${BASE_DIR}/bundles.yml" | grep -q "true") ||
	   (yq eval ".quarterly | has(\"${_PRODUCT_VERSION}\")" "${BASE_DIR}/bundles.yml" | grep -q "true")
	then
