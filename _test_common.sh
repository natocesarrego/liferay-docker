...
function _display_test_initial_message {
	if [ -n "${BASH_SOURCE[3]}" ]
	then
		echo -e "\n### Running tests from $(echo "${BASH_SOURCE[3]}" | sed --regexp-extended --expression "s/\.\///g") ###\n"
	elif [ -n "${BASH_SOURCE[2]}" ]
	then
		echo -e "\n### Running tests from $(echo "${BASH_SOURCE[2]}" | sed --regexp-extended --expression "s/\.\///g") ###\n"
	fi
}
