...
function main {
	...
	_build_jdk "JDK 11" "jdk11" "11"
	_build_jdk "JDK 11/JDK 8" "jdk11-jdk8" "8"
	_build_jdk "JDK 21" "jdk21" "21"
...
function _build_jdk {
	...
	LIFERAY_DOCKER_IMAGE_PLATFORMS="${LIFERAY_DOCKER_IMAGE_PLATFORMS}" LIFERAY_DOCKER_REPOSITORY="${LIFERAY_DOCKER_REPOSITORY}" LIFERAY_DOCKER_ZULU_11_AMD64_VERSION=${latest_available_zulu_amd64_version} LIFERAY_DOCKER_ZULU_11_ARM64_VERSION=${latest_available_zulu_arm64_version} time ./build_"${jdk_image_name//-/_}"_image.sh "${BUILD_ALL_IMAGES_PUSH}" | tee -a "${LIFERAY_DOCKER_LOGS_DIR}"/"${jdk_image_name}".log
